...
object SweepPrinter {
    ...
    fun printPlayerSweep(results: Map<Int, BatchStatistics>, gamesPerConfig: Int, roundsPerGame: Int) {
        ...
        println()
        println("  ${"Players".padEnd(10)} ${"Avg".padStart(7)} ${"Gini".padStart(7)} ${"Skl+".padStart(7)} ${"A-C".padStart(7)} ${"Farm%".padStart(7)} ${"Via".padStart(7)} ${"Eggs/Rnd".padStart(10)}")
        println("  ${"─".repeat(70)}")
        ...
        for ((nPlayers, stats) in results.toSortedMap()) {
            println("  ${nPlayers.toString().padEnd(10)} ${
                ...
                "%.1f".format(stats.nightMetrics.avgEggsCreatedPerRound).padStart(10)
            }")
        }
        println()
    }
    ...
    fun printRoundSweep(results: Map<Int, BatchStatistics>, gamesPerConfig: Int, nPlayers: Int) {
        ...
        println()
        println("  ${"Rounds".padEnd(10)} ${"Avg".padStart(7)} ${"Gini".padStart(7)} ${"Skl+".padStart(7)} ${"A-C".padStart(7)} ${"Farm%".padStart(7)} ${"Via".padStart(7)}")
        println("  ${"─".repeat(55)}")
